...
namespace GK6X {
	public static class Crc16 {
		...
		static Crc16() {
			...
			for (var i = 0; i < table.Length; i++) table[i] = BitConverter.ToUInt16(byteTable, i * 2);
		}

		public static ushort GetCrc(byte[] data, int index = 0, ushort iv = 0xFFFF) {
			var crc = iv;
			for (var i = index; i < data.Length; i++) {
				var tableOffset = (byte) (data[i] ^ (byte) (crc >> 8));
		...
		public static void InsertCrc(byte[] data, int dataOffset = 0, int crcOffset = 6) {
			data[crcOffset + 0] = 0;
			data[crcOffset + 1] = 0;
			var dataCrc = GetCrc(data, dataOffset);
		...
		public static bool ValidateCrc(byte[] data, int dataOffset = 0, int crcOffset = 6) {
			var dataCrc = BitConverter.ToUInt16(data, crcOffset);
			var temp1 = data[crcOffset + 0];
			var temp2 = data[crcOffset + 1];
			data[crcOffset + 0] = 0;
			data[crcOffset + 1] = 0;
			var calculatedCrc = GetCrc(data, dataOffset);
