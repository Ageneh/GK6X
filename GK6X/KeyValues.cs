...
namespace GK6X {
	public static class KeyValues {
		...
		public static bool Load() {
			...
			var filePath = Path.Combine(Program.DataBasePath, fileName);
			...
			foreach (DriverValue value in Enum.GetValues(typeof(DriverValue)))
				if (GetKeyType((uint) value) == DriverValueType.Key &&
					...
					var shortValue = GetKeyData1((uint) value);
			...
			if (File.Exists(filePath)) {
				...
				var keyGroups = Json.Deserialize(File.ReadAllText(filePath)) as List<object>;
				foreach (var keyGroupObj in keyGroups) {
					var keyGroupMembers = keyGroupObj as Dictionary<string, object>;
					var group = new Group();
					Groups.Add(group);
					...
					if (Json.TryGetValue(keyGroupMembers, "keys", out keyGroupKeys))
						...
						foreach (var lineKeysListObj in keyGroupKeys) {
							var lineKeysListDict = lineKeysListObj as Dictionary<string, object>;
							List<object> lineKeysObjs;
							if (Json.TryGetValue(lineKeysListDict, "linekeys", out lineKeysObjs))
								foreach (var lineKeysObj in lineKeysObjs) {
									var keyInfo = lineKeysObj as Dictionary<string, object>;

									var key = new Key(group);
									long locationCode;
									long logicCode;
									...
									if (Json.TryGetValue(keyInfo, "DriverValue", out driverValueStr) &&
									    !string.IsNullOrEmpty(driverValueStr)) {
										if (driverValueStr.StartsWith("0x")) {
											long driverValue;
											if (long.TryParse(driverValueStr.Substring(2), NumberStyles.HexNumber, null,
												out driverValue)) {
												key.DriverValue = (uint) driverValue;
												if (Keys.ContainsKey(key.DriverValue))
													Debug.WriteLine("[WARNING] Duplicate key 0x" +
													                key.DriverValue.ToString("X8") + " (" + fileName +
													                ")");
											...
											var splitted = driverValueStr.Split(driverValueSplitChars,
	...
	internal static class MacroKeyNames {
		...
		static MacroKeyNames() {
			...
			Add("Num Enter", DriverValue.NumPadEnter,
				"NumpadEnter"); // <--- The official software just maps to regular "Enter"
		...
		private static void Add(string name, DriverValue value, string jsCode) {
			Names[name] = new Item {
				Name = name,
